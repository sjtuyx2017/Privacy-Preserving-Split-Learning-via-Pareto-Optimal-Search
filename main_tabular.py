...
from process_data.load_health import load_health
from process_data.load_adult import load_adult
from models.models_health import Encoder1, Classifier1, Discriminator1,Decoder1,Encoder_Info1, Encoder2, Classifier2, Discriminator2,Decoder2,Encoder_Info2
from models.models_adult import Adult_Encoder, Classifier, discriminator, Decoder,Info_Encoder_a

from sklearn.metrics import multilabel_confusion_matrix,confusion_matrix
from utils.G_mean import G_mean_DPA
...
parser.add_argument('--dataset', default='adult',choices=['health', 'adult'], type=str)
parser.add_argument('--utility_task', default='Income', choices=['Charlson', 'Income'],)
parser.add_argument('--privacy_task', default='Relationship', choices=['Gender','Age','Sex','Education','Relationship'],)
parser.add_argument('--lr', default=1e-3, type=float)
parser.add_argument('--batch_size', default=256, type=int)     # health:128   adult:500
parser.add_argument('--num-features', default=128, type=int)    # health:128   adult:32
parser.add_argument('--iterations', default=1000, type=int)
parser.add_argument('--defense', default='GMM-RO', choices=['GMM-LC', 'GMM-EPO', 'GMM-RO', 'ADV', 'Noisy', 'Infocensor', 'None'], type=str)
parser.add_argument('--model_idx', default=2, choices=[1, 2, 3], type=int)  # different model partitions :


parser.add_argument("--beta1", type=float, default=0.8, help="adam: decay of first order momentum of gradient")
...
# Noisy parameter
parser.add_argument('--noise_sigma', default=1000, type=float)   # deciding the variance of the noise

# Infocensor parameter
parser.add_argument('--info_lambda', default=0.3, type=float)
...
parser.add_argument('--preference', default=0.1, type=float) # preference for accuracy task
parser.add_argument('--eps', default=1e-3, type=float) # EPO non-uniformity restriction

# GMM-RO parameter
parser.add_argument('--warmup_iterations', default=100, type=float)
parser.add_argument('--privacy_constraint', default=0.005, type=float)
...
makedir(SavePath)
SavePath = SavePath + '/pr=%s_defense=%s_iter=%s_bs=%s'%(
   str(args.poison_ratio), args.defense, str(args.iterations), str(args.batch_size))

if args.defense == 'GMM-LC':
    SavePath = SavePath + '_sigma=%s_acc-w=%s' % (str(args.sigma), str(args.acc_weight))
elif args.defense == 'GMM-EPO':
    SavePath = SavePath + '_sigma=%s_pref=%s_pc=%s_mode=%s' %(str(args.sigma),
        str(args.preference), str(args.privacy_constraint), str(args.restrict_mode))
...
class Client(nn.Module):
    def __init__(self, encoder, adv_decoder):
        ...
        self.privacy_criterion = self.privacy_criterion = GMMLoss_pca(args.sigma, 0)
        ...
        self.optimizer_encoder = optim.Adam(self.encoder.parameters(), lr=args.lr, betas=(args.beta1, args.beta2),
    ...
    def get_feature_ADV(self, x_private, private_label=None, training=0):
        if training:
            ...
            opposite_label = private_label_num-predicted-1
...
class Cloud(nn.Module):
    ...
    def batch_attack(self, z_private, private_label, decoder):
        ...
        # if args.privacy_task == 'Gender' or 'Sex':
        cm = confusion_matrix(private_label.cpu(), predicted.cpu())   # 二分类
        # else:
        # cm = multilabel_confusion_matrix(private_label.cpu(), predicted.cpu())  # 多分类
    ...
    def train_step(self, z_private, x_public, private_label, public_label, label, attacker_label):
        ...
        DSAdecoder_loss = self.accuracy_criterion(DSAdecoder_outputs_public, public_label.long())
...
def attack(client, cloud, train_loader, test_loader):
    ...
    # if args.privacy_task == 'Gender' or 'Sex':
    # train_DSA_Gmean = sqrt(train_DSA_cm[0][0] / (train_DSA_cm[0][0] + train_DSA_cm[0][1]) *
    #                        train_DSA_cm[1][1] / (train_DSA_cm[1][0] + train_DSA_cm[1][1]))
    # train_DPA_Gmean = sqrt(train_DPA_cm[0][0] / (train_DPA_cm[0][0] + train_DPA_cm[0][1]) *
    #                        train_DPA_cm[1][1] / (train_DPA_cm[1][0] + train_DPA_cm[1][1]))
    train_DSA_Gmean = 0
    train_DPA_Gmean = 0
    # else:
    # train_DSA_Gmean = G_mean_DPA(train_DSA_cm)
    # train_DPA_Gmean = G_mean_DPA(train_DPA_cm)
...
def train(client, cloud, train_loader, test_loader, decoder_loader):
    ...
    for i in tqdm.tqdm(iterator, total=args.iterations):
        ...
        if (i + 1) % interval == 0:
            ...
            train_set_DSA_acc, train_set_DPA_acc, train_DSA_Gmean, train_DPA_Gmean = attack(client, cloud,
                                                                                            train_loader,
                                                                                            test_loader)
            ...
            if args.defense == 'GMM-LC':
                logger.info('%d \t\t %.4f \t\t %.4f \t\t %.4f \t\t %.4f \t\t %.4f \t\t %.4f \t\t %.4f \t\t %.4f',
                            ...
                            train_acc, test_acc, train_set_DSA_acc, train_DSA_Gmean, train_set_DPA_acc,
                            train_DPA_Gmean
                            )
            elif args.defense == 'GMM-EPO' or args.defense == 'GMM-RO':
                logger.info(
    ...
    if args.defense != 'None':
        ...
        plot_and_save_figure(train_pri_losses, [], 'batch', 'loss', 'train privacy loss', priSavePath,
                             'privacy_loss.png')
    ...
    plot_and_save_figure(train_acc_list, [], 'batch', 'accuracy', 'train accuracy', accSavePath,
                         'train_accuracy.png')
    ...
    plot_and_save_figure(train_ftilde_losses, [], 'batch', 'loss', 'train f_tilde loss', DSASavePath,
                         'train_ftilde_loss.png')
    plot_and_save_figure(train_DSAdecoder_losses, [], 'batch', 'loss', 'train DSA decoder loss', DSASavePath,
                         'DSAdecoder_loss.png')
    plot_and_save_figure(train_D_losses, [], 'batch', 'loss', 'train D loss', DSASavePath, 'train_D_loss.png')
    plot_and_save_figure(train_set_DSA_acc_list, [], 'batch', 'accuracy', 'train set DSA accuracy', DSASavePath,
                         'train_set_DSA_acc.png')
...
if __name__ == '__main__':
    ...
    attr = args.privacy_task.lower()
    if dataset == 'health':
        train_loader, test_loader, decoder_loader, input_dim, label_num, private_label_num = load_health(attr=attr,
                                                                                                         batch_size=batch_size)
    elif dataset == 'adult':
        train_loader, test_loader, decoder_loader, input_dim, label_num, private_label_num = load_adult(attr=attr,
                                                                                                        batch_size=batch_size)
    ...
    if dataset == 'health':
        if args.defense == 'Infocensor':
            encoder = Encoder_Info2(in_dim=input_dim, out_dim=args.num_features, drop_rate=args.drop_rate)
        else:
            encoder = Encoder2(in_dim=input_dim, out_dim=args.num_features, drop_rate=args.drop_rate)

        top_model = Classifier2(in_dim=args.num_features, num_classes=label_num, drop_rate=args.drop_rate)
        f_tilde = Encoder2(in_dim=input_dim, out_dim=args.num_features, drop_rate=0.1)
        DPAdecoder = Decoder2(in_dim=args.num_features,
                           num_classes=private_label_num,
                           drop_rate=0.0)
        DSAdecoder = Decoder2(in_dim=args.num_features,
                              num_classes=private_label_num,
                              drop_rate=0.0)
        D = Discriminator2(in_dim=args.num_features)
        adv_decoder = Decoder2(in_dim=args.num_features,
                               num_classes=private_label_num,
                               drop_rate=0.0)

    elif dataset == 'adult':
        if args.defense == 'Infocensor':
            encoder = Info_Encoder_a(in_dim=input_dim)
        else:
            encoder = Adult_Encoder(in_dim=input_dim)
        top_model = Classifier(num_classes=label_num)
        f_tilde = Adult_Encoder(in_dim=input_dim)
        DPAdecoder = Decoder(num_classes=private_label_num)
        DSAdecoder = Decoder(num_classes=private_label_num)
        D = discriminator()
        adv_decoder = Decoder(num_classes=private_label_num)
