...
parser.add_argument('--dataset', default='UTKFace', choices=['celebA', 'UTKFace'], type=str)
parser.add_argument('--cloud_task', default='gender', type=str)
parser.add_argument('--attack_task', default='white', type=str)
...
parser.add_argument('--defense', default='GMM-ADV', choices=['GMM-LC', 'GMM-EPO', 'GMM-ADV', 'ADV', 'None'], type=str)
...
# GMM-LC combination parameter
parser.add_argument('--acc_weight', default=0.9, type=float)
...
directory = './%s_NewGMMLoss_average_results'%(args.dataset)
makedir(directory)
...
if args.defense == 'GMM-LC':
    ...
    SavePath = SavePath + '_adv-w=%s' % (str(args.adv_weight))
...
class Client(nn.Module):
    ...
    def get_feature_ADV(self, x_private, private_label, training):
        if training:
            self.encoder.train()
    ...
    def get_feature_GMM(self, x_private, private_label, training, batch_idx):
        if training:
            self.encoder.train()
            x_private = x_private.to(device)
    ...
    def get_feature_GMM_ADV(self, x_private, private_label, training):
        if training:
            self.encoder.train()
            self.adv_decoder.train()
            x_private, private_label = x_private.to(device), private_label.to(device)
    ...
    def update_GMM_ADV(self):
        self.top_gradient, self.flatten_tg = save_and_clear_grad(self.encoder)
        merged_gradient = merge_gradients(self.top_gradient, self.privacy_gradient, [args.acc_weight, args.acc_weight])
    ...
    with torch.no_grad():
        for batch_idx, (x, label, private_label) in enumerate(test_loader):
            ...
            if args.defense == 'None':
                ...
                z_private = client.get_feature_GMM(x, private_label, 0, batch_idx)
            elif args.defense == 'GMM-ADV':
                z_private = client.get_feature_GMM_ADV(x, private_label, 0)
...
def train(client, cloud, train_loader, test_loader, decoder_loader):
    ...
    if args.defense == 'GMM-LC' or args.defense == 'GMM-EPO':
        logger.info('Iteration \t Train Acc Loss \t Train GMM Loss \t Train Acc \t Test Acc \t Test Attack Acc')
    elif args.defense == 'ADV':
        logger.info('Iteration \t Train Acc Loss \t Train Adversary Loss \t Train Acc \t Test Acc \t Test Attack Acc')
    elif args.defense == 'GMM-ADV':
        logger.info('Iteration \t Train Acc Loss \t Train GMM Loss \t Train Adversary Loss \t Train Acc \t Test Acc \t Test Attack Acc')
    else:
        logger.info('Iteration \t Train Acc Loss \t  Train Acc \t Test Acc \t Test Attack Acc')
    ...
    for i in tqdm.tqdm(iterator, total=args.iterations):
        ...
        if (i+1) % interval == 0:
            ...
            test_attack_acc = atk(client, cloud, train_loader, test_loader)
            ...
            if args.defense == 'GMM-LC' or args.defense == 'GMM-EPO':
                logger.info('%d \t\t %.4f \t\t %.4f \t\t %.4f \t\t %.4f \t\t %.4f',
                            ...
                            train_acc , test_acc, test_attack_acc
                            )
            elif args.defense == 'ADV':
                logger.info('%d \t\t %.4f \t\t %.4f \t\t %.4f \t\t %.4f \t\t %.4f',
    ...
    if args.attack != 'None':
        ...
        test_attack_acc_list = smooth(test_attack_acc_list, WINDOW_SIZE)
        ...
        plot_and_save_figure(test_attack_acc_list, [], 'batch', 'accuracy', 'attack accuracy', topSavePath, 'attack_acc.png')
