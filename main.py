...
from utils.GMM_loss import GMMLoss, GMMLoss_pca, GMMLoss2_pca
...
parser.add_argument('--defense', default='None', choices=['GMM-LC', 'GMM-EPO', 'ADV', 'Noisy', 'Infocensor', 'None'], type=str)
...
parser.add_argument('--info_lambda', default=1.0, type=float)
parser.add_argument('--info_beta', default=0.0, type=float)
parser.add_argument('--fair_kappa', default=1.0, type=float)
...
parser.add_argument('--restrict_mode', default='None',choices=['start', 'batch', 'None'],
                    type=str)  # whether the privacy loss constraint acts on each batch
parser.add_argument('--pca_dim', default=256, type=int)

# GMM-LC parameters
parser.add_argument('--acc_weight', default=0.1, type=float)
...
if args.defense == 'GMM-LC':
    ...
    SavePath = SavePath + '_loss=%s_sigma=%s_pref=%s_eps=%s_pc=%s_mode=%s' %(str(args.loss_type), str(args.sigma),
        str(args.preference), str(args.eps), str(args.privacy_constraint), str(args.restrict_mode))
...
class Client(nn.Module):
    def __init__(self, encoder, adv_decoder):
        ...
        if args.loss_type == 1:
            self.privacy_criterion = GMMLoss_pca(args.sigma, args.pca_dim, device)
        elif args.loss_type == 2:
            self.privacy_criterion = GMMLoss2_pca(args.sigma, args.pca_dim, device)
        if args.model_idx == 3:
...
def train(client, cloud, train_loader, test_loader, decoder_loader):
    ...
    train_acc_losses = np.array([])
    train_pri_losses = np.array([])
    train_adv_losses = np.array([])

    # DSA losses
    train_ftilde_losses = np.array([])
    train_DSAdecoder_losses = np.array([])
    train_D_losses = np.array([])

    # DPA losses
    train_DPAdecoder_losses = np.array([])

    # train and attack accuracy
    train_acc_list = np.array([])
    train_DSA_acc_list = np.array([])
    train_DPA_acc_list = np.array([])
    test_acc_list = np.array([])
    train_set_DSA_acc_list = np.array([])
    train_set_DPA_acc_list = np.array([])
    #test_set_attack_acc_list = np.array([])
    train_set_DSA_Gmean_list = np.array([])
    train_set_DPA_Gmean_list = np.array([])
    ...
    for i in tqdm.tqdm(iterator, total=args.iterations):
        ...
        if args.defense == 'None':
            ...
            train_adv_losses = np.append(train_adv_losses, batch_adv_loss)
        ...
        train_ftilde_losses = np.append(train_ftilde_losses, batch_ftilde_loss)
        train_acc_loss += batch_acc_loss
        train_acc_losses = np.append(train_acc_losses, batch_acc_loss)
        train_acc_list = np.append(train_acc_list, batch_correct / args.batch_size)
        train_DSAdecoder_losses = np.append(train_DSAdecoder_losses, batch_DSAdecoder_loss)
        train_D_losses = np.append(train_D_losses, batch_D_loss)
        train_DSA_acc_list = np.append(train_DSA_acc_list, batch_DSA_correct / args.batch_size)
        train_DPAdecoder_losses = np.append(train_DPAdecoder_losses, batch_DPAdecoder_loss)
        train_DPA_acc_list = np.append(train_DPA_acc_list, batch_DPA_correct / args.batch_size)
        ...
        if (i+1) % interval == 0:
            ...
            train_set_DSA_acc_list = np.append(train_set_DSA_acc_list, train_set_DSA_acc)
            train_set_DPA_acc_list = np.append(train_set_DPA_acc_list, train_set_DPA_acc)
            #test_set_attack_acc_list = np.append(test_set_attack_acc_list, test_set_attack_acc)
            train_set_DSA_Gmean_list = np.append(train_set_DSA_Gmean_list, train_DSA_Gmean)
            train_set_DPA_Gmean_list = np.append(train_set_DPA_Gmean_list, train_DPA_Gmean)
            ...
            if args.defense == 'GMM-LC' or args.defense == 'GMM-EPO':
                ...
                logger.info('%d \t\t %.4f \t\t %.4f \t\t %.4f \t\t %.4f \t\t %.4f \t\t %.4f \t\t %.4f \t\t %.4f',
                            (i + 1), train_acc_loss / interval, train_info_loss / interval,
                            train_acc, test_acc, train_set_DSA_acc, train_DSA_Gmean, train_set_DPA_acc, train_DPA_Gmean
                            )
            else:
